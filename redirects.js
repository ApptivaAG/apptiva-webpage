 ...
 *
 * Extracted from next.config.js to keep the config readable.
 * All redirects are permanent (308). When adding entries, group them by topic
 * (legacy blog posts, /referenzen → /projekte, /angebot landing pages, etc.).
 *
 * @type {() => Promise<import('next').NextConfig['redirects'] extends (() => Promise<infer R>) ? R : never>}
 */
